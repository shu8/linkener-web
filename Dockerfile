...
## - service worker registration need to define the scope to '/' (see https://stackoverflow.com/a/48068714)
...
RUN if [ "$BASE_PATH" = "/" ]; then \
        ...
        find __sapper__/export -type f -name "*.html" -exec sed -i "s|navigator.serviceWorker.register('\/\(.*\)service-worker.js')|navigator.serviceWorker.register(\'/\1service-worker.js',{scope:'/'})|g" {} \;; \
    fi
